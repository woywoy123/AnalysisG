...
class RecursiveGraphNeuralNetwork(MessagePassing):
    ...
    def __init__(self):
        ...
        self.rnn_update = Seq(
                Linear(self._o*2, self._o*2),
                LayerNorm(self._o*2),
                Linear(self._o*2, self._o)
        )
        self.rnn_update.apply(init_norm)
    ...
    def forward(self,
        ...
        hashx: str = str((self.pmc/1000).view(-1).sum(-1).item()) + "-" + str(self._cls)
        if self._nuR and hashx not in self._cache:
            ...
            self._cache[hashx] = self.pmu

        elif hashx in self._cache:
            self.pmu = self._cache[hashx]
            self.pmc = pyc_cuda.combined.transform.PxPyPzE(self.pmu)
        ...
        while True:
            ...
            G: Tensor = self._h[idx_mlp[edge_index_[0], edge_index_[1]]]
            H = self.rnn_update(torch.cat([H, H - G], -1))
