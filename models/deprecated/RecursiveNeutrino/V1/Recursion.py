...
class Recursion(MessagePassing):
    ...
    def message(self, edge_index, pmc_i, pmc_j, Pmc_i, Pmc_j):
        ...
        mlp_i__ = self._RecurMLP(
            torch.cat([mlp_i_ - mlp_j_, mlp_i - mlp_j, mlp_i_, self._i[msk_]], -1)
        )
    ...
    def forward(
        ...
        self._i = self._RecurMLP(
            torch.cat([mlp_i_ - mlp_j_, mlp_i - mlp_j, mlp_i_, zero_], -1)
        )
        ...
        while True:
            ...
            self._i = self._RecurMLP(
                torch.cat([mlp_i_ - mlp_j_, mlp_i - mlp_j, mlp_i_, self._i], -1)
            )
