...
class Base(MessagePassing):
    def __init__(self):
        ...
        self._MLP = Seq(Linear(1 + end + 2, end), Tanh(), ReLU(), Linear(end, 2))
        self._Feat = Seq(Linear(4, end), Tanh(), ReLU(), Linear(end, end))
    ...
    def message(self, edge_index, pmc_i, pmc_j, pmc__i, pmc__j, mlp_ji, mlp_i, mlp_j):
        ...
        dR_ji = PCC.DeltaR(
            pmc_i[:, 0], pmc_j[:, 0], pmc_i[:, 1], pmc_j[:, 1], pmc_i[:, 2], pmc_j[:, 2]
        )
        dR__ji = PCC.DeltaR(
            pmc_i[:, 0],
            pmc__j[:, 0],
        ...
        mlp_ji = self._MLP(
            torch.cat(
                [(self._it + 1) * (edge - 0.5), Edge_ji, mlp_ji - mlp_i - mlp_j], -1
            )
        )
    ...
    def forward(self, edge_index, Pmc):
        ...
        edge = torch.ones_like(
            torch.cat([degree(dst).view(-1, 1) for i in range(2)], -1)
        )[:, 0].view(-1, 1)

        mass_n = PCC.Mass(pmc)
        mass_ji, mass_j = (
            PCC.Mass(pmc[src] + pmc[dst] * (src != dst).view(-1, 1)),
            PCC.Mass(pmc)[src],
        )
        ...
        mlp = self._MLP(
            torch.cat([(self._it + 1) * (edge - 0.5), Mass_n, -edge, edge], -1)
        )
        mlp_ji = self._MLP(
            torch.cat(
                [
                    (self._it + 1) * (edge[src] - 0.5),
                    Mass_ji,
                    mlp[src] - mlp[src] - mlp[dst],
                ],
                -1,
            )
        )
        for i in torch.arange(it):
            mlp_ji, mlp, pmc = self.propagate(
                edge_index, pmc=Pmc, pmc_=pmc, mlp_ji=mlp_ji, mlp=mlp
            )
            self._it += 1
        return mlp_ji
...
class GraphPassing(MessagePassing):
    def __init__(self):
        ...
        self._MLP = Seq(Linear(8, end), Tanh(), ReLU(), Linear(end, 2))
    ...
    def forward(
        self, edge_index, batch, N_pT, N_eta, N_phi, N_energy, G_met, G_met_phi
    ):
