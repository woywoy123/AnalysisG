...
class GraphPassing(MessagePassing):
    ...
    def message(self, edge_index, pmc_j, pmc__i, mlp_i, mlp_j):
        src, dst = edge_index
        m_i, m_ij = PCC.Mass(pmc__i), PCC.Mass(pmc__i + pmc_j)
        mlp_ij = self._MLP(torch.cat([m_i, m_ij, mlp_j - mlp_i], -1))
        sel = (mlp_ij.max(-1)[1]*(src != dst)).view(-1, 1)
        return pmc_j*sel, mlp_ij

    def aggregate(self, message, index, pmc, pmc_, mlp):
        (pmc_j, mlp_ij), m_i = message, PCC.Mass(pmc_) 
            
        dst_ = index.unique()
        mlp_ij_sum = self.aggr_module_add(mlp_ij, index)/(degree(index).view(-1, 1))
        pmc_ = pmc_.clone()
        pmc_[dst_] += self.aggr_module_add(pmc_j, index)
        mlp = self._MLP(torch.cat([PCC.Mass(pmc), PCC.Mass(pmc_), mlp_ij_sum - mlp], -1))
        return mlp_ij, mlp, pmc_
    ...
    def forward(self, i, num_nodes, batch, edge_index, N_pT, N_eta, N_phi, N_energy, G_met, G_met_phi, E_T_edge):
        ...
        src, dst = edge_index
        mass = PCC.Mass(Pmc)
        zero_ = torch.zeros_like(torch.cat([pt for i in range(2)], -1)) 
        ...
        for i in range(12):
            ...
            mlp_ij, mlp, pmc_ = self.propagate(edge_index, pmc = Pmc, pmc_ = pmc_, mlp = mlp)
            mlp_ij = self._MLP(torch.cat([mass_, PCC.Mass(pmc_[src]), mlp[src] - mlp_ij], -1))
