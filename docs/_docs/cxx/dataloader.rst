.. cpp:function:: bool dataloader::dump_graphs(std::string path, int threads)

    Dumps the graphs in the data set to HDF5 files.
...
.. cpp:function:: std::map<std::string, graph_t*>* dataloader::restore_graphs_(std::vector<std::string> cache_, int threads)

    Restores graphs from a list of cache paths.
...
.. cpp:function:: void dataloader::restore_graphs(std::vector<std::string> path, int threads)

    Restores graphs from a list of HDF5 files.
...
.. cpp:function:: void dataloader::restore_graphs(std::string path, int threads)

    Restores graphs from a directory containing HDF5 files.
...
.. cpp:function:: dataloader::dataloader()
...
.. cpp:function:: void dataloader::shuffle(std::vector<int>* idx)

    Shuffles the elements of a vector of integers.
...
.. cpp:function:: void dataloader::shuffle(std::vector<graph_t*>* idx)

    Shuffles the elements of a vector of graph pointers.

    This function shuffles the elements of the input vector using a Fisher-Yates shuffle algorithm.

    :param idx: A pointer to the vector of graph pointers to be shuffled.

.. cpp:function:: void dataloader::clean_data_elements(std::map<std::string, int>** data_map, std::vector<std::map<std::string, int>*>* loader_map)

    Cleans and manages data element maps.
...
.. cpp:function:: void dataloader::extract_data(graph_t* gr)

    Extracts data from a graph object and adds it to the data set.
...
.. cpp:function:: void dataloader::datatransfer(torch::TensorOptions* op, size_t* num_ev, size_t* cur_evnt)

    Transfers graph data to a specified device.
...
.. cpp:function:: void dataloader::datatransfer(std::map<int, torch::TensorOptions*>* ops)

    Transfers graph data to multiple devices.
...
.. cpp:function:: std::vector<graph_t*>* dataloader::build_batch(std::vector<graph_t*>* data, model_template* mdl, model_report* rep)

    Builds a batch of graphs for training or evaluation.
...
.. cpp:function:: void dataloader::safe_delete(std::vector<graph_t*>* data)

    Safely deletes a vector of graph pointers and their associated data.
...
.. cpp:function:: void dataloader::cuda_memory_server()

    Manages CUDA memory on the server.
...
.. cpp:function:: void dataloader::start_cuda_server()

    Starts the CUDA memory server.
...
.. cpp:function:: std::vector<graph_t*> dataloader::get_random(int num)

    Gets a random subset of graphs from the data set.
...
.. cpp:function:: void dataloader::generate_kfold_set(int k)

    Generates k-fold cross-validation sets.
...
.. cpp:function:: void dataloader::generate_test_set(float percentage)

    Generates a test set by splitting the data set.
...
.. cpp:function:: std::vector<graph_t*>* dataloader::get_k_train_set(int k)

    Gets the training set for a specific fold in k-fold cross-validation.
...
.. cpp:function:: std::vector<graph_t*>* dataloader::get_k_validation_set(int k)

    Gets the validation set for a specific fold in k-fold cross-validation.
...
.. cpp:function:: std::vector<graph_t*>* dataloader::get_test_set()
...
.. cpp:function:: std::map<std::string, std::vector<graph_t*>>* dataloader::get_inference()

    Gets the data for inference, sorted by filename.
...
.. cpp:function:: void dataloader::dump_dataset(std::string path)

    Dumps the dataset information to a file.
...
.. cpp:function:: bool dataloader::restore_dataset(std::string path)

    Restores the dataset information from a file.
