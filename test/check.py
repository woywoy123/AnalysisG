...
def MakeTensor_(inpt, indx):
    return torch.tensor([[ i[indx].px/1000, i[indx].py/1000, i[indx].pz/1000, i[indx].e/1000 ] for i in inpt], dtype = torch.float64, device = "cuda")
...
def AssertEquivalenceList(truth, pred, threshold = 0.0001):
    for i, j in zip(truth, pred):
        if AssertEquivalence(i, j, threshold): continue
        return False
    return True
...
def AssertSimilarSets(pred1, pred2, threshold):
    if len(pred1) != len(pred2): return False
    p1, p2 = [], []
    for i, j in zip(pred1, pred2):
        if sum(i) != 0: p1.append(i)
        if sum(j) != 0: p2.append(j)
    for i in p1:
        diff1 = {}
        for k in p2: diff1[sum([abs((l1 -l2)/l1)*100 for l1, l2 in zip(k, i)])**0.5] = [k, i] 
        l = list(diff1)
        l.sort()
        trig = False
        if l[0] > threshold: trig = True
...
def ParticleToTensor(part, device = "cuda"):
    return torch.tensor([[part.px, part.py, part.pz, part.e]], device = device, dtype = torch.float64)
...
class SampleTensor:
    ...
    def __init__(self, b, mu, ev, top, device = "cpu", S = [[100, 0], [0, 100]]):
        ...
        self.mT = torch.tensor([[top[i][0].Mass/1000] for i in range(self.n)], device = self.device, dtype = torch.float64)
