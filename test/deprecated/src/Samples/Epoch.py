...
class Epoch(EpochPlots, Metrics, Tools):
    ...
    def MakeDictionary(self, name):
        ...
        setattr(
            self,
            "ROC_" + name,
            {k[2:]: {"truth": [], "p_score": []} for k in self.ModelOutputs},
        )
    ...
    def Collect(self, pred, truth, loss_acc, name):
        ...
        for key in ROC:
            ROC[key]["truth"] += (
                truth[key].view(-1).detach().cpu().to(dtype=torch.int).tolist()
            )
    ...
    def Process(self, MakeFigure=True):
        ...
        self._Package[self.Epoch] |= {
            "Accuracy_" + i: self.MakeStatics(self.__dict__["Accuracy_" + i])
            for i in self.names
        }
        self._Package[self.Epoch] |= {
            "Loss_" + i: self.MakeStatics(self.__dict__["Loss_" + i])
            for i in self.names
        }
        self._Package[self.Epoch] |= {
            "TotalLoss_" + i: self.MakeStatics(self.__dict__["TotalLoss_" + i])
            for i in self.names
        }
        self._Package[self.Epoch] |= {"EpochTime": self.EpochTime}
        self._Package[self.Epoch] |= {
            "AUC_" + i + "_" + k: self.__dict__["ROC_" + i][k]["auc"]
            for i in self.names
            for k in self.__dict__["ROC_" + i]
        }
        self._Package[self.Epoch] |= {"NodeTimes": self.MakeStatics(self.NodeTimes)}
        self._Package[self.Epoch] |= {
            "FoldTime": self.MakeStatics([i for i in self.FoldTime.values()])
        }
    ...
    def Compile(self, outdir):
        ...
        for i in Merge:
            if len(Merge[i].yData) != 0:
                ...
                Combined |= {
                    i + "_" + str(j): Merge[i].FeatureContainer[j]
                    for j in Merge[i].FeatureContainer
                }
        ...
        for i in Combined:
            ...
            if i == "EpochTime":
                ...
                cont.Plots = self.FoldTimePlot(
                    epochs, cont.yData, outdir, cont.errData, cont.errData
                )
                Compile[i] = cont

            elif i.startswith("Accuracy"):
                cont.Plots = self.AccuracyPlot(
                    epochs,
                    cont.yData,
                    outdir,
                    i.split("_")[1],
