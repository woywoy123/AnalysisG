...
class Interface:
    ...
    def InputSample(self, Name, SampleDirectory=None):
        if self._launch == False:
            self._InputValues.append(
                {"INPUTSAMPLE": {"Name": Name, "SampleDirectory": SampleDirectory}}
            )
            return
        ...
        if isinstance(SampleDirectory, str):
            if SampleDirectory.endswith(".root"):
                SampleDirectory = {
                    "/".join(SampleDirectory.split("/")[:-1]): [
                        SampleDirectory.split("/")[-1]
                    ]
                }
            else:
        ...
        if isinstance(SampleDirectory, dict):
            ...
            return
        self._SampleMap[Name] |= self.ListFilesInDir(
            {i: "*" for i in SampleDirectory}, ".root"
        )
    ...
    def EvaluateModel(self, Directory, ModelInstance=None, BatchSize=None):
        ...
        self._ModelDirectories[Name] = [
            self.abs(Directory + "/" + i) for i in self.ls(Directory) if "Epoch-" in i
        ]
...
class Analysis(Interface, Analysis_, Settings, SampleTracer, GraphFeatures, Tools):
    ...
    def __BuildRootStructure(self):
        ...
        self._tmp = self.pwd()
        if (
            (self.DumpPickle or self.DumpHDF5)
            or self.TrainingSampleName
            or (len(self._MSelection) > 0 or len(self._Selection) > 0)
        ):
    ...
    def __GenerateEvents(self, InptMap, name):
        ...
        if (
            self.EventCache
            and reE == False
            and self.DumpPickle == False
            and self.DumpHDF5 == False
        ):
            ...
            if self.EventGraph != None and reD == False:
                ...
                if f not in self:
                    if self.Event == None:
                        ...
                        continue
                    self.SampleContainer += self.__EventGenerator(
                        name, tmp
                    ).SampleContainer
    ...
    def __DumpCache(self, instance, name, fdir, Compiled):
        ...
        if Compiled == False:
            th = Threading(
                [ev.Filename for ev in instance],
                QuickExportEvent,
                self.Threads,
                self.chnk,
            )
            ...
            export = {
                hsh: instance.SampleContainer._Hashes[hsh]
                for hsh, ev in th._lists
                if ev
            }
            del th
        else:
            lst = [
                i.split(".")[0]
                for i in self.ls(self.output + "/DataCache/" + name + "/" + fdir + "/")
            ]
    ...
    def __GenerateTrainingSample(self):
        ...
        if self.Training == False:
            ...
            hashes["train_hashes"] = {
                hsh: self.HashToROOT(hsh) for hsh in hashes["train_hashes"]
            }
            hashes["test_hashes"] = {
                hsh: self.HashToROOT(hsh) for hsh in hashes["test_hashes"]
            }
    ...
    def __SearchAssets(self, CacheType, Name):
        ...
        tracers = set(
            [tr + "/" + i + ".pkl" for i in self.ls(smpls)]
            + [tr + "/" + i for i in self.ls(tr)]
        )
        samples = set(
            [smpls + "/" + i for i in self.ls(smpls)]
            + [smpls + "/" + i.replace(".pkl", "") for i in self.ls(tr)]
        )
        if len(tracers) == 0:
            self.MissingTracer(Name)
            return False
        th = Threading(
            [i for i in tracers if i.split("/")[-1].replace(".pkl", "") not in self],
            function,
            self.Threads,
            int(len(tr) / self.chnk) + 1,
        )
    ...
    def __Selection(self):
        ...
        for i in self._Selection:
            ...
            th = Threading(
                [[self._Selection[i], k, i] for k in self],
                _select,
                self.Threads,
                self.chnk,
            )
        ...
        for i in self._MSelection:
            l = [
                self.output + "/Selections/" + i + "/" + k
                for k in self.ls(self.output + "/Selections/" + i)
            ]
            ...
            th.Start()
            pkl.PickleObject(
                sum([k for k in th._lists if k != None]),
                i,
                self.output + "/Selections/Merged/",
            )
    ...
    def Launch(self):
        ...
        if self.TrainingSampleName:
            fDir = (
                self.output
                + "/Tracers/TrainingSample/"
                + self.TrainingSampleName
                + ".pkl"
            )
            self.Training = UnpickleObject(fDir) if self.IsFile(fDir) else False
            self._SampleMap = (
                self.Training["SampleMap"] if self.Training else self._SampleMap
            )
    ...
    def __iter__(self):
        ...
        smpls = {
            i["INPUTSAMPLE"]["Name"]: None
            for i in self._InputValues
            if "INPUTSAMPLE" in i
        }
        self._lst = {
            "Data": iter(self.SampleContainer.list()),
            "Tracers": None,
            "Current": [],
        }
        if len(smpls) != 0 and CacheType != None and Dumped:
            ...
            self._lst["Tracers"] = [
                cache + i + "/" + j for i in smpls for j in self.ls(cache + i)
            ]
            self._lst["Data"] = [
                i.replace("/Tracers", "").replace(".pkl", "")
                for i in self._lst["Tracers"]
            ]
            self._lst["Data"] = iter(
                [[i + "/" + j for j in self.ls(i)] for i in self._lst["Data"]]
            )
    ...
    def __next__(self):
        event = (
            next(self._lst["Data"])
            if len(self._lst["Current"]) == 0
            else self._lst["Current"].pop(0)
        )
        if isinstance(event, list):
            events = {
                i.split("/")[-1].replace(".hdf5", "").replace(".pkl", ""): i
                for i in event
            }
