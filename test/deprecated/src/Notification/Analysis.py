...
class Analysis(Notification):
    ...
    def MissingTracer(self, Directory):
        ...
        _str += (
            " please enable either 'DumpPickle' or 'DumpHDF5'."
    ...
    def FoundFiles(self, Files):
        ...
        for i in Files:
            root = (
                True if len([l for l in Files[i] if l.endswith(".root")]) > 0 else False
            )
            string = ""
    ...
    def AddedSelection(self, Name):
        self.Success(
            "Added Selection ("
            + Name
            + ") to Analysis. The output will be within the project folder under 'Selections/"
            + Name
            + "'."
        )
