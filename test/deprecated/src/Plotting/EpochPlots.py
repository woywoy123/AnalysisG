...
class EpochPlots:
    ...
    def CompileROC(self, Names, ModelOutputs):
        ...
        for feat in [i[2:] for i in ModelOutputs]:
            ...
            Plots = MakePlot()
            Plots["OutputDirectory"] = (
                self.OutDir + "/Epoch-" + str(self.Epoch) + "/ROC"
            )
    ...
    def AccuracyHistograms(self, ModelOutputs):
        for feat in [i[2:] for i in ModelOutputs]:
            ...
            Plots["Title"] = (
                "Accuracy Distribution for Different Samples Feature: " + feat
            )
            Plots["Histograms"] = Hists
    ...
    def AccuracyPlot(self, xData, yData, outdir, mode, feature, up, down):
        Plots = self.TemplateLine(
            xData, [i for i in yData], outdir + "/Accuracy", mode, up, down
        )
    ...
    def MergeLoss(self, Loss, Colors, Markers, outdir, Subdir="Loss"):
        ...
        Plots["yMax"] = max(
            [
                j + k
                for i in Loss["Total"]
                for j, k in zip(i.Plots.yData, i.Plots.up_yData)
            ]
        )
