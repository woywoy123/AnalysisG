...
def GetSourceCode(obj):
    ...
    except: return inspect.getsource(obj.__class__)


def GetSourceFile(obj):
    if obj.__class__.__name__ == "type": return GetSourceCode(obj)
    try: return "".join(open(inspect.getfile(obj.__class__), "r").readlines())
    except: return inspect.getsource(obj)

def GetSourceFileDirectory(obj):
    try: return os.path.abspath(sys.modules[obj.__module__].__file__)
    except AttributeError: return obj.__package__
