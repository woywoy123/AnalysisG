def mapping(name):
    ...
    if "_ttee."       in name: return ( "ttee"      , "$tt\\ell\\ell$"        )
    if "_ttmumu."     in name: return ( "ttmumu"    , "$tt\\ell\\ell$"        )
    if "_tttautau."   in name: return ( "tttautau"  , "$tt\\ell\\ell$"        )
    if "_ttW."        in name: return ( "ttW"       , "ttW"                   )
    if "_ttZnunu."    in name: return ( "ttZnunu"   , "$ttZ\\nu\\nu$"         )
    if "_ttZqq."      in name: return ( "ttZqq"     , "ttZqq"                 )
    if "_tW."         in name: return ( "tW"        , "tW"                    )
    if "_tZ."         in name: return ( "tZ"        , "tZ"                    )
    if "_Wenu_"       in name: return ( "Wenu"      , "Wev"                   )
    if "_WH125."      in name: return ( "WH125"     , "WH"                    )
