...
class Particle:
    def __init__(self, pt = None, eta = None, phi = None, e = None, px = None, py = None, pz = None):
        self.pt  = pt
        self.eta = eta
        self.phi = phi
        self.px  = px
    ...
    def vec(self):
        if self.pt is not None:
            dct = {"pt" : self.pt, "eta" : self.eta, "phi" : self.phi, "energy" : self.e}
        else: dct = {"px" : self.px, "py" : self.py, "pz" : self.pz}
...
class event:
    ...
    @property
    def vec(self): return vector.obj(pt = self.met, phi = self.phi)
