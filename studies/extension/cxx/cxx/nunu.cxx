...
nunu_solver::nunu_solver(std::vector<wrapper*>* targets, double met, double phi){
    this -> _metx = met; //std::cos(phi) * met;  
    this -> _mety = phi; //std::sin(phi) * met; 
...
void nunu_solver::solve(){
    ...
    std::map<double, std::tuple<nusol*, nusol*>>::iterator itr = this -> pairings.begin(); 
