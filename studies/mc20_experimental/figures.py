...
def enforce_sym_jet(p, m, pgd):
    ...
    if len(set(absx)) <= 2: return False
    if p.count("b") and p.count("2"): return False
    if p.count("b") and p.count("3"): return False
    if p.count("g"): return False

    if   p.count("nu") and p.count("e") == 2: return True
    elif p.count("nu") and p.count("mu") == 2: return True
    elif not p.count("nu"): return not (p.count("e") + p.count("mu"))
    else: return False
    return True
...
def entry_point(fancy, mode, pth, tree, plt = True):
    ...
    topx  = top_decay_stats(tops    ,  "p_ntops",  "p_ltops",  "p_htops", fnc[0], fancy, pths[0], plt)
    topc  = top_decay_stats(children,  "c_ntops",  "c_ltops",  "c_htops", fnc[1], fancy, pths[1], plt)
    toptj = top_decay_stats(truthjet, "tj_ntops", "tj_ltops", "tj_htops", fnc[2], fancy, pths[2], plt)
    topjc = top_decay_stats(jetschil, "jc_ntops", "jc_ltops", "jc_htops", fnc[3], fancy, pths[3], plt)
    topjl = top_decay_stats(jetslept, "jl_ntops", "jl_ltops", "jl_htops", fnc[4], fancy, pths[4], plt)
    ...
    top_mass_dist(truthjet, "tj", fnc[2], fancy, pths[2], plt,   0, 300, 300, 10.0)
    top_mass_dist(jetschil, "jc", fnc[3], fancy, pths[3], plt,   0, 300, 300, 10.0)
    top_mass_dist(jetslept, "jl", fnc[4], fancy, pths[4], plt,   0, 300, 300, 10.0)
    ...
    toptj_nj = constrain_top_njets(truthjet, "tj", fnc[2], fancy, pths[2], plt, 0, 400, 200, 20.0)
    topjc_nj = constrain_top_njets(jetschil, "jc", fnc[3], fancy, pths[3], plt, 0, 400, 200, 20.0)
    topjl_nj = constrain_top_njets(jetslept, "jl", fnc[4], fancy, pths[4], plt, 0, 400, 200, 20.0)
...
def entry(smpls, path):
    ...
    if "BSM_4TOPS" in smpls: entry_point("Reference MC16"     , "reference-mc16"   , path + "bsm_4tops/*"        , "nominal"      , True)
