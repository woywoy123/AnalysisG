...
class Ellipse(cosmetic):
    ...
    def make(self, n_points = 100, pt = 0):
...
#        if vl[0] < 0 or vl[1] < 0: return None, None, None
        ...
        if not n_points: return self.equation(a, b, pt)
    ...
    def plot(self):
        ...
        self.max_z, self.min_z = np.max(z), np.min(z)
        self._plot3(x, y, z)
        if self.ptx is None: return
        sx, sy, sz = self.make(0, self.ptx)
        self.marker_size = 12
        self.color = "r"
        self._scatter3(sx, sy, sz)
