...
class NuSol(traject):
    def __init__(self, b, mu, evc, ev, mW2, mT2, mN2 = 0):
        traject.__init__(self, evc)
        self._b = b
        self._mu = mu
        self.mW2 = mW2
        self.mT2 = mT2
    ...
    @property
    def x0p(self):
        return -(self.mT2 - self.mW2 - self.b.mass2)/(2*self.b.e)

    @property
    def x0(self):
        return -(self.mW2 - self.mu.mass2 - self.mN2)/(2*self.mu.e)

    @property
    def Sx(self):
        beta = self.mu.b
        return (self.x0 * beta - self.mu.p * (1 - beta**2))/beta**2

    @property
    def Sy(self):
        return ((self.x0p / self.b.b) - self.c * self.Sx) / self.s
    ...
    @property
    def BaseMatrix(self):
        x = (self.b.mass >= 0)*(self.mu.mass >= 0)
        m = x*np.array([
    ...
    @property
    def H(self): 
        m = self.R_T.dot(self.BaseMatrix)
        return m
    ...
    def N(self):
        ...
        m = hinv.T.dot(UnitCircle()).dot(hinv)
        return m
    ...
    @plx.setter
    def plx(self, lx):
        self.polx = self.figures(Ellipse, self.color, 2.0) 
        self.ellipse(lx, self.polx)
        self.polx.alpha = 1.0
...
class DoubleNu:
    ...
    def nunu_s(self):
        ...
        for i in range(len(nu1)):
            ...
            p1 = particle(nu2[i][0], nu2[i][1], nu2[i][2])
            out[0].append(p1)
            out[1].append(p2)
        return out
