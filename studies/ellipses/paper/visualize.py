...
class cosmetic:
    ...
    def _contour(self, x, y, z):
        self.ax.contour(x, y, z, cmap = self.color, levels = 120)
...
class figure(cosmetic):
    ...
    def axis_lims(self, dim, min_, max_):
        if not self.auto_lims: return
        if dim == "x": self.ax.set_xlim(min_, max_, auto = True)
        if dim == "y": self.ax.set_ylim(min_, max_, auto = True)
        if dim == "z": self.ax.set_zlim(min_, max_, auto = True)

    def plot_title(self, name, font_size): 
        self.ax.set_title(name, fontsize = font_size)

    def show(self):
        self.plot_title(self.title, 20)
