...
class Conuix(variables):
    ...
    def komega(self):
        ...
        print(np.linalg.eig(kpsi)[-1])#.dot(kpsi))
    ...
    def mass_neutrino(self, sign):
        cn = signs(self.c_psi_p, self.c_psi_m, sign)
        sn = signs(self.s_psi_p, self.s_psi_m, sign)
        tn = signs(self.t_psi_p, self.t_psi_m, sign)
        w  = signs(self.wp, self.wm, sign)
        o  = signs(self.op, self.om, sign)
        kp = w * sn - cn + self.lep.b
        km = w * sn - cn - self.lep.b

        return complex(self.m_mu ** 2 + (self.p_mu * o * sn)**2 / (kp * km)) ** 0.5

    def mass_nu(self, s1, s2):
        cn = signs(self.c_psi_p, self.c_psi_m, s1)
        sn = signs(self.s_psi_p, self.s_psi_m, s1)
        tn = signs(self.t_psi_p, self.t_psi_m, s1)
    ...
    def Tilde(self, x1, y1, z1, si):
        w = signs(self.wp, self.wm, si)
        o = signs(self.op, self.om, si)
        g = np.array([
            [z1 / o    , 0, x1 - self.p_mu ],
            [z1 * w / o, 0, y1             ],
            [0         , z1,              0],
