...
class branches:
    ...
    def tau(self, Sx, Sy):
        S = np.array([[Sx], [Sy]])
        S = np.linalg.inv(self.M).dot(S - self.S0).reshape(2)
        print(S)
        S = ((S[1] / S[0])) 
        if abs(S) > 1: S = 1 / S; 
        return math.atanh( S )
...
class line:
    ...
    def __init__(self, m, b, x0 = None, y0 = None, name = None, var = None, sign1 = None, sign2 = None):
        ...
        if x0 is not None and y0 is not None: 
            sx, sy, sz = var.SolveSxSy(x0, y0, var.mass_neutrino(sign2), sign2)
...
class variables(debug):
    ...
    def AK(self, delta, sign):
        w = signs(self.wp, self.wm, sign)
        o = signs(self.op, self.om, sign)
        delta = 1 / delta
        s = self.lep.b**2 - w**2 + 2 * w * delta - (1 - self.lep.b**2) * delta**2
        return s / o**2
