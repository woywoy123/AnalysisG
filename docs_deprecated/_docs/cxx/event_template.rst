.. cpp:file:: event_template.h
    ...
    The `event_template` class provides a comprehensive and flexible structure for handling various aspects of event-related information. This includes:
...
.. cpp:class:: event_template : public tools
    ...
    This class serves as a blueprint for representing and manipulating individual events in a data analysis context. It encapsulates event properties, associated particles, and the logic required to build and process event information from various data sources.
    ...
    .. cpp:function:: event_template()
        :brief: Default constructor for the `event_template` class.

        Initializes the `event_template` object. This typically involves setting up the `cproperty` members by associating them with their respective internal storage and potentially default values, as well as linking them to their static setter and getter methods where applicable. It ensures the object is in a valid initial state.

    .. cpp:function:: virtual ~event_template()
        :brief: Virtual destructor for the `event_template` class.

        Ensures proper cleanup when an `event_template` object (or a derived class object) is destroyed. This includes invoking `flush_particles()` to release resources held by associated particle objects and potentially deregistering any particle generators that were dynamically created or managed by this event instance. The virtual nature allows derived classes to implement their specific cleanup logic.

    .. cpp:member:: cproperty<std::vector<std::string>, event_template> trees
        :brief: Configurable property representing the list of TTree names relevant to this event template.

        This `cproperty` manages a `std::vector<std::string>` containing the names of the TTrees (from input files, typically ROOT files) that this event template might interact with or read data from. It uses `set_trees` as its setter function.

    .. cpp:function:: static void set_trees(std::vector<std::string>* inpt, event_template* ev)
        :brief: Static setter function for the `trees` cproperty.

        This function is called internally by the `trees` cproperty when its value is set. It updates the internal list of tree names (`m_trees`) within the provided `event_template` instance based on the input vector.
    ...
    .. cpp:member:: cproperty<std::vector<std::string>, event_template> branches
        :brief: Configurable property representing the list of TBranch names relevant to this event template.

        This `cproperty` manages a `std::vector<std::string>` containing the names of the TBranches (within the specified TTrees) that this event template needs to access. It uses `set_branches` as its setter function.

    .. cpp:function:: static void set_branches(std::vector<std::string>* inpt, event_template* ev)
        :brief: Static setter function for the `branches` cproperty.

        This function is called internally by the `branches` cproperty when its value is set. It updates the internal list of branch names (`m_branches`) within the provided `event_template` instance based on the input vector.
    ...
    .. cpp:member:: cproperty<std::vector<std::string>, event_template> leaves
        :brief: Configurable property representing the list of TLeaf names relevant to this event template.

        This `cproperty` manages a `std::vector<std::string>` containing the names of the TLeaves (within the specified TBranches) that hold the actual data values this event template will use. It uses `get_leaves` as its getter function. Note that direct setting might be handled differently, potentially via `add_leaf`.

    .. cpp:function:: static void get_leaves(std::vector<std::string>* inpt, event_template* ev)
        :brief: Static getter function for the `leaves` cproperty.

        This function is called internally by the `leaves` cproperty when its value is requested. It populates the provided vector with the current list of leaf names stored internally (`m_leaves`).
    ...
    .. cpp:member:: cproperty<std::string, event_template> name
        :brief: Configurable property representing the logical name of the event template.

        This `cproperty` manages a `std::string` that holds a user-defined name for this specific event configuration or type. It uses `set_name` as its setter function. This name can be useful for identification and debugging.

    .. cpp:function:: static void set_name(std::string* name, event_template* ev)
        :brief: Static setter function for the `name` cproperty.

        This function is called internally by the `name` cproperty when its value is set. It updates the internal name storage within the provided `event_template` instance.
    ...
    .. cpp:function:: static void get_hash(std::string* val, event_template* ev)
        :brief: Static getter function for the `hash` cproperty.

        This function is called internally by the `hash` cproperty when its value is requested. It retrieves the stored hash value from the `event_template` instance.
    ...
    .. cpp:member:: cproperty<std::string, event_template> tree
        :brief: Configurable property representing the name of the primary or current TTree being processed.

        This `cproperty` manages a `std::string` holding the name of the TTree that is currently considered active or the primary source for the event data. It uses `set_tree` as its setter and `get_tree` as its getter. This is useful when an event might be composed of data from multiple trees, specifying the main one.

    .. cpp:function:: static void set_tree(std::string* name, event_template* ev)
        :brief: Static setter function for the `tree` cproperty.

        This function is called internally by the `tree` cproperty when its value is set. It updates the currently active tree name within the `event_template` instance.
    ...
    .. cpp:function:: static void get_tree(std::string* name, event_template* ev)
        :brief: Static getter function for the `tree` cproperty.

        This function is called internally by the `tree` cproperty when its value is requested. It retrieves the name of the currently active tree from the `event_template` instance.
    ...
    .. cpp:member:: cproperty<double, event_template> weight
        :brief: Configurable property representing the weight associated with the event.

        This `cproperty` manages a `double` value representing the event weight, often used in statistical analysis (e.g., for Monte Carlo simulations or data corrections). It uses `set_weight` as its setter function.

    .. cpp:function:: static void set_weight(double* inpt, event_template* ev)
        :brief: Static setter function for the `weight` cproperty.

        This function is called internally by the `weight` cproperty when its value is set. It updates the event weight within the `event_template` instance.
    ...
    .. cpp:member:: cproperty<long, event_template> index
        :brief: Configurable property representing the index or entry number of the event in the original data source.

        This `cproperty` manages a `long` integer representing the sequential index (often the entry number in a TTree) of the event within its original file or dataset. It uses `set_index` as its setter function.

    .. cpp:function:: static void set_index(long* inpt, event_template* ev)
        :brief: Static setter function for the `index` cproperty.

        This function is called internally by the `index` cproperty when its value is set. It updates the event index within the `event_template` instance.
    ...
    .. cpp:member:: std::map<std::string, std::string> m_trees
        ...
        This map holds the definitive list of tree names relevant to this event template, possibly mapping user-friendly keys to actual TTree names. It is populated via the `set_trees` method (called by the `trees` cproperty).
    ...
    .. cpp:member:: std::map<std::string, std::string> m_branches
        ...
        This map holds the definitive list of branch names relevant to this event template, possibly mapping user-friendly keys to actual TBranch names. It is populated via the `set_branches` method (called by the `branches` cproperty).
    ...
    .. cpp:function:: bool operator==(event_template& p)
        ...
        Compares the current `event_template` object with another one (`p`). The comparison is based solely on the equality of their respective hash values (`hash` cproperty). This provides a way to check if two event templates likely represent the same configuration.
    ...
    .. cpp:member:: event_t data
        ...
        This member variable, of type `event_t` (defined in `structs/event.h`), likely contains the low-level or aggregated data extracted from the input source for the current event after the `build` process. It might store simple data types or pointers/references to more complex structures.

    .. cpp:member:: meta* meta_data = nullptr
        :brief: Pointer to associated metadata.

        Holds a pointer to a `meta` object (defined in `meta/meta.h`). This object likely contains metadata related to the dataset or sample from which the event originates (e.g., cross-section, luminosity, sample type). Initialized to `nullptr`.

    .. cpp:member:: std::string filename = ""
        :brief: The name of the file from which the current event was read.

        Stores the filename (potentially including the path) of the input data file that contains the current event record. Useful for tracking data provenance and debugging. Initialized to an empty string.

    .. cpp:function:: void flush_particles()
        :brief: Cleans up resources associated with particles managed by this event.

        This method iterates through all registered particle generators (stored in `particle_generators`) and likely calls a cleanup function (e.g., `clear()` or similar) on each associated particle collection. This is crucial for resetting the event state between processing different entries or for releasing memory before destruction. It is called by the destructor and potentially at the beginning of the `build` method.

    .. cpp:function:: void build_mapping(std::map<std::string, data_t*>* evnt)
        :brief: Builds the internal mapping between tree/branch/leaf names and data elements. (Private)

        This private helper method is likely called during the initialization or configuration phase (e.g., within `build_event`). It examines the structure of the input data sources (represented by the `evnt` map) and establishes the `tree_variable_link` map. This link connects the specified tree names to maps that associate variable/leaf names with their corresponding `element_t` representations, facilitating efficient data access during the `build` phase.
    ...
    .. cpp:function:: void flush_leaf_string()
        :brief: Clears internal string representations related to leaves. (Private)

        This private helper method is likely used to reset or clear any temporary string buffers or cached representations related to leaf names or values, possibly as part of the `flush_particles` or event reset logic. The exact purpose depends on the internal implementation details.

    .. cpp:member:: std::map<std::string, bool> next_ = {}
        :brief: Map to track the processing state (e.g., "next entry needed") for each associated tree. (Private)

        This map likely stores a boolean flag for each TTree name associated with the event template. The flag might indicate whether the next entry needs to be read from that specific tree during the event building process, especially when dealing with friend trees or data spread across multiple TTrees.

    .. cpp:member:: std::map<std::string, particle_template*> particle_generators
        :brief: Map storing pointers to the actual particle generator objects (base class pointers). (Private)
    ...
    .. cpp:member:: std::map<std::string, std::map<std::string, element_t>> tree_variable_link = {}
        :brief: Nested map linking tree names to variable names and their corresponding data element structures. (Private)

        This is a crucial internal mapping structure built by `build_mapping`. It allows the `build` method to quickly find the `element_t` structure corresponding to a specific variable (leaf/branch) name within a given tree context.
    ...
    .. cpp:member:: std::map<std::string, std::map<std::string, particle_template*>*> particle_link = {}
        :brief: Map linking particle type names to the maps containing the actual particle instances. (Private)
