...
__global__ void Sq_ij_K(
        ...
        const unsigned int length, const unsigned int i, const unsigned int j
){
...
__global__ void SumK(
        torch::PackedTensorAccessor64<scalar_t, 2, torch::RestrictPtrTraits> pmc,
        const unsigned int length, const unsigned int dim_len
){
    const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x; 
...
__global__ void SqrtK(
        torch::PackedTensorAccessor64<scalar_t, 2, torch::RestrictPtrTraits> pmc,
        const unsigned int length, const unsigned int dim_min, const unsigned int dim_max)
{
    const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void Sqrt_i_K(
        torch::PackedTensorAccessor64<scalar_t, 2, torch::RestrictPtrTraits> pmc, 
        const unsigned int length, const unsigned int dim_i)
{
    const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x; 
    if (idx >= length){ return; }
...
__global__ void ArcCos_ij_K(
        ...
        const unsigned int length, const unsigned int dim_i, const unsigned int dim_j)
{
    const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x; 
    if (idx >= length){ return; }
