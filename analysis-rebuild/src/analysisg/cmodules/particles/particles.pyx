...
from .particles cimport particles
from analysisg.cmodules.structs.structs cimport particle_t
from analysisg.cmodules.tools.tools cimport env, enc

cdef class ParticleTemplate:

    def __cinit__(self):
        self.ptr = new particles()
        self.children = []
        self.parents = []

    def __init__(self): pass
    def __dealloc__(self): del self.ptr
    def __hash__(self): return int(self.ptr.hash().substr(0, 8), 0)
...
# 
    ...
    def LeptonicDecay(self) -> bool:
        ...
        for x in self.Children: ch.push_back(x.ptr.data)
