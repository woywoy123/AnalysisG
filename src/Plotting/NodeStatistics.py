...
class SampleNode:
    ...
    def Process(self):
        for tr in self.SampleNodes:
            out = []
            out, tbl = self.MakeSummary(
                    self.SampleNodes[tr], 
                    "All Samples: Node Summary by Process for " + tr + " tree", 
                    out)

            out, tbl = self.MakeSummary(
                    self.Training[tr], 
                    "Training Samples: Node Summary by Process for " + tr + " tree", 
                    out)

            out, tbl = self.MakeSummary(
                    self.TestSample[tr], 
                    "Test Samples: Node Summary by Process for " + tr + " tree", 
                    out)
            tbl.output = out
