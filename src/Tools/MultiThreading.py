...
class ProgressiveThreading(_MultiThreading):
    def __init__(self, Function, threads: Union[int], chunk_size: Union[int]):
        ...
        # threads being executed
        self._running = {}

        # progress bar
        self._dct = {}

    def AddData(self, process: Union[list]):
        self._chain += [process]
        if len(self._chain) > self._chunk_size:
            self._it += 1
            self._backlog[self._it] = self._chain[:self._chunk_size]
            self._chain = self._chain[self._chunk_size:]
            self.Processing()

    def End(self):
        self._backlog[self._it+1] = self._chain
        self.Processing()
        while len(self._running): self.CheckThreads()
        del self._chain
        self._Close()
        final = []
        for i in self.done: final += self.done[i]
        return final

    def Processing(self):
        if not len(self._dct): self._Progress()
        if len(self._running) >= self._threads:
            return self.CheckThreads()
        for i in self._backlog:
            if self._backlog[i] is None: continue
            if len(self._running) < self._threads:
                self.NewThread(i)
            else: self.CheckThreads()

    def CheckThreads(self):
        for i in list(self._running):
            th, recv, tmpl = self._running[i]
            if not recv.poll(): continue
            res = recv.recv()
            if not res and isinstance(res, bool):
                self.RecoveredThread(tmpl.i)
                self.done[i] = tmpl.MainThread()
                continue
            self.done[i] = res
            recv.send(True)
            recv.close()
    ...
    def NewThread(self, index):
        T = TemplateThreading(self._function)
        T.lock = torch.multiprocessing.Manager().Lock()
        T.i = (index%self._threads)+1
        self._running[index] = [None, None, T]
        recv, send = Pipe(True)
        P = Process(target = self._running[index][2].Exec, args=(send,))
        self._running[index][0] = P
        self._running[index][1] = recv
        P.start()

        self._running[index][1].send(self._backlog[index])
        self._running[index][2].started = True
        self._running[index][2]._i = [index, self._backlog[index]]
        self._backlog[index] = None
