...
class AnalysisScript(AnalysisG.Tools.General.Tools, Settings):
    ...
    def Compile(self):
        ...
        for i in self.Config:
            ...
            if isinstance(self.Config[i], str): script += ["<*Analysis*>." + i + " = " + "'" + str(self.Config[i]) + "'"] 
            else: script += ["<*Analysis*>." + i + " = " + str(self.Config[i])] 
       
        for i in self.Code:
            if i == "Event": 
                shared["imports"] += ["from " + self.Code[i]._Name + " import " + self.Code[i]._Name]
                script += ["<*Analysis*>.Event = " + self.Code[i]._Name]
                shared["code"][self.Code[i]._Name] = ["from AnalysisG.Templates import EventTemplate"]
                shared["code"][self.Code[i]._Name] += ["from " + list(self.Code["Particles"].values())[0]._File.split("/")[-1].replace(".py", "") + " import "]
                shared["code"][self.Code[i]._Name][-1] += ", ".join([p._Name for p in self.Code["Particles"].values()])
...
class Condor(AnalysisG.Tools.General.Tools, _Condor, Settings):
    ...
    def DumpCondorJobs(self):
        ...
        for j in self._Complete:
            if self._Complete[j] == True: continue 
            self._Jobs[j].ImportSettings(self)
            self._Jobs[j].DumpConfig()
            self._Complete[j] = True 
