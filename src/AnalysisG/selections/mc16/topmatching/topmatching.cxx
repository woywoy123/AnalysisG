...
bool topmatching::strategy(event_template* ev){
    ...
    for (size_t x(0); x < dtops.size(); ++x){
        ...
        if (ch.size()){
            ...
            if (ch_.size()){this -> truth_children["lep"].push_back(this -> sum(&ch));}
        ...
        if (top_i -> TruthJets.size()){
            ...
            if (ch_.size()){this -> truth_jets["lep"].push_back(mass);}
            ...
            if (ch_.size()){this -> n_truth_jets_lep[ntj].push_back(mass);}
        ...
        if (ch_.size()){this -> jets_truth_leps["lep"].push_back(mass_j);}    
        ...
        for (size_t c(0); c < dleps.size(); ++c){
            ...
            if (!pr.count(top_i -> hash)){continue;}
        ...
        if (ch_.size()){this -> jet_leps["lep"].push_back(mass_j_);}
        ...
        if (ch_.size()){this -> n_jets_lep[_ntj].push_back(mass_j_);}
