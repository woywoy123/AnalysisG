...
std::map<std::string, torch::Tensor> nusol_::combinatorial(
    ...
    // build matrix
    long dx_s = steps; 
    const dim3 thmss  = dim3(32, 32); 
    const dim3 blkmss = blk_(dx_s, 32, dx_s, 32);
    torch::Tensor massTW1 = torch::rand({dx_s*dx_s, 2}, MakeOp(pmc)); 
    torch::Tensor massTW2 = torch::rand({dx_s*dx_s, 2}, MakeOp(pmc)); 
    ...
    AT_DISPATCH_ALL_TYPES(pmc -> scalar_type(), "massx", [&]{
        _mass_matrix<<<blkmss, thmss>>>(
                massTW1.packed_accessor64<double, 2, torch::RestrictPtrTraits>(), mTl, msT, mWl, msW, dx_s
        ); 
        _mass_matrix<<<blkmss, thmss>>>(
                massTW2.packed_accessor64<double, 2, torch::RestrictPtrTraits>(), mTl, msT, mWl, msW, dx_s
        ); 
    ...
    std::map<std::string, torch::Tensor> H1_m = nusol_::BaseMatrix(&pmcb1, &pmcl1, &massTW1); 
    torch::Tensor H1      = H1_m["H"]; //reshaped(&H1_m["H"]     , &ix, dx_s, 0); 
    torch::Tensor H1_inv  = H1_m["H_perp"]; //reshaped(&H1_m["H_perp"], &ix, dx_s, 0); 
    H1_inv = std::get<0>(operators_::Inverse(&H1_inv)); 

    std::map<std::string, torch::Tensor> H2_m = nusol_::BaseMatrix(&pmcb2, &pmcl2, &massTW2); 
    torch::Tensor H2      = H2_m["H"]; //reshaped(&H2_m["H"]     , &ix, dx_s, 1); 
    torch::Tensor H2_inv  = H2_m["H_perp"]; //reshaped(&H2_m["H_perp"], &ix, dx_s, 1); 
    H2_inv = std::get<0>(operators_::Inverse(&H2_inv)); 
    torch::Tensor skp = ((torch::abs(H1_inv) > 0)*(torch::abs(H2_inv) > 0)).sum({-1}).sum({-1}); 
    ...
    torch::Tensor xt = (torch::ones({ev.size({0}), dx_s*dx_s}, MakeOp(batch)).cumsum({0})-1); 
