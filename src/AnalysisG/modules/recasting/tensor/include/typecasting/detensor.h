...
namespace AnalysisG {
    namespace typecasting {
        ...
        template <typename g>
        void as_vector(std::vector<g>* trgt, std::vector<g>* chnks, std::vector<signed long>*, int); 

        template <typename G, typename g>
        void as_vector(std::vector<G>* trgt, std::vector<g>* chnks, std::vector<signed long>* dims, int next_dim = 0); 

        template <typename g>
        void as_vector(torch::Tensor* data, std::vector<g>* out); 

        template <typename G, typename g>
        bool as_vector(torch::Tensor* data, std::vector<G>* out, std::vector<signed long>* dims, g); 


        template <typename G, typename g>
        torch::Tensor as_tensor(std::vector<G>* _data, at::ScalarType _op, g, torch::TensorOptions* op); 

    }
}

#include "as_vector.inl" 
#include "as_tensor.inl" 
