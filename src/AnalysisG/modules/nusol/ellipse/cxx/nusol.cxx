...
wrapper::wrapper(particle_template* p_){
    this -> px = p_ -> px;
    this -> py = p_ -> py;
    this -> pz = p_ -> pz;
    this -> e  = p_ -> e; 
...
nuelx::nuelx(){}
nuelx::nuelx(particle_template* b_, particle_template* l_, double mW, double mT){
    this -> b = new wrapper(b_); 
    this -> l = new wrapper(l_);
    this -> mw = mW; 
    this -> mt = mT; 
    this -> mw2 = mW*mW;
    this -> mt2 = mT*mT; 
...
void nuelx::Z2(double* A, double* B, double* C){
...
void nuelx::r_mW(double* mw1_, double* mw2_){
    double p_l  = this -> l -> p;
    double p_b  = this -> b -> p; 
    double e_l  = this -> l -> e; 
    double e2l  = e_l*e_l; 
...
void nuelx::r_mT(double* mt1_, double* mt2_){
   *mt1_ = -1; *mt2_ = -1; 
   double c = this -> _c; 
   double s = this -> _s; 
   double w_ = this -> w();
...
void nuelx::Z_mW(double* mw1_, double* mw2_){
    double w_ = this -> w();
    double o2 = this -> om2();
    ...
    double X0 = sx * (1 - 1/o2) - (w_ * sy0) / o2; 
    double X1 = E1 * (1 - 1/o2) - (w_ * sy1) / o2; 
...
void nuelx::Z_mT(double* mt1_, double* mt2_, double _mw){
...
void nuelx::update(double mt_, double mw_){
