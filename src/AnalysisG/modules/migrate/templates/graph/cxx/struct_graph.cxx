...
graph_t::graph_t(){}
graph_t::~graph_t(){this -> _purge_all();}
...
void graph_t::_purge_data(std::map<int, torch::Tensor*>* data){this -> mflush(data);}
...
void graph_t::meta_serialize(std::vector<torch::Tensor*>* data, std::string* out){
    ...
    *out = this -> encode64((const unsigned char*)out -> c_str(), out -> size()); 
}
...
void graph_t::meta_deserialize(std::map<std::string, int>* data, std::string* out){
    if ((*out) == "NULL"){return;}
    std::vector<std::string> line = this -> split(*out, "%"); 
    for (size_t x(0); x < line.size()-1; ++x){
        std::vector<std::string> c = this -> split(line[x], "|"); 
...
void graph_t::meta_deserialize(std::vector<torch::Tensor*>* data, std::string* out){
    if ((*out) == "NULL"){return;}
    std::string tmp = this -> decode64(out); 
