...
void model_template::save_state(){
    auto lamb = [this](
            torch::OrderedDict<std::string, torch::Tensor>* prm,
            torch::serialize::OutputArchive* session
    ) -> void {
        for (torch::OrderedDict<std::string, torch::Tensor>::Item& val : *prm) {
            std::string key = val.key();
            key = this -> encode64(&key);
            session -> write(key, val.value());
        }
    }; 
    ...
    for (size_t x(0); x < this->m_data.size(); ++x) {
        ...
        params = this -> m_data[x] -> ptr().get() -> named_parameters(true);
        lamb(&params, &state_archive); 
        params = this -> m_data[x] -> ptr().get() -> named_buffers(true);
        lamb(&params, &state_archive); 
        state_session.write("sequential_b64_at_" + std::to_string(x), state_archive);
    }
...
bool model_template::restore_state(){
    auto lamb = [this](
            torch::OrderedDict<std::string, torch::Tensor>* prm,
            torch::serialize::InputArchive* session
    ) -> void {
        for (torch::OrderedDict<std::string, torch::Tensor>::Item& val : *prm) {
            std::string key = val.key();
            key = this -> encode64(&key);
            torch::Tensor restore;
            session -> read(key, restore);
            val.value().copy_(restore);
        }
    }; 
    ...
    for (size_t x(0); x < this -> m_data.size(); ++x){
        ...
        state_session.read("sequential_b64_at_" + std::to_string(x), state_archive);

        torch::NoGradGuard no_grad;
        torch::nn::Module* base_module = this -> m_data[x] -> ptr().get();
        torch::OrderedDict<std::string, torch::Tensor> params; 
        params = base_module -> named_parameters(true);
        lamb(&params, &state_archive); 
        params = base_module -> named_buffers(true);
        lamb(&params, &state_archive); 
    }
