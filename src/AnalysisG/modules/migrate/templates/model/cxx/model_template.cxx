...
model_template::~model_template(){
    ...
    tools::vflush(&this -> m_data); 
    this -> flush_outputs(); 
    this -> m_data.clear(); 
    
    tools::plush(&this -> m_loss); 
    lamb(&this -> m_o_graph); 
    lamb(&this -> m_o_node); 
    lamb(&this -> m_o_edge); 
}
...
void model_template::flush_outputs(){
    ...
    tools::mflush(&this -> m_p_graph, true); 
    tools::mflush(&this -> m_p_node , true); 
    tools::mflush(&this -> m_p_edge , true); 
    tools::mflush(&this -> m_p_undef, true); 
    this -> m_p_loss.clear(); 
