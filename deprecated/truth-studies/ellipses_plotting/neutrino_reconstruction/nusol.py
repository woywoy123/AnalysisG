...
class NuSol(object):
    ...
    def Sx(self):
        P = self.mu.mag
        beta = self.mu.beta
        beta2 = 2*beta - 1 if abs(1 - beta) < 1e-4 else beta**2
        betam2 = 3 - 2*beta if abs(1 - beta) < 1e-4 else 1/beta2
        return (self.x0 * beta - P * (1 - beta2))*betam2
 
    @property
    def Sy(self):
        beta = self.b.beta
        betam1 = 2 - beta if abs(1 - beta) < 1e-4 else 1/beta
        return ((self.x0p*betam1) - self.c * self.Sx) / self.s

    @property
    def w(self):
        beta_m, beta_b = self.mu.beta, self.b.beta
        beta_bm1 = 2 - beta_b if abs(1 - beta_b) < 1e-4 else 1/beta_b
        return (beta_m*beta_bm1 - self.c)/self.s

    @property
    def Om2(self): 
        beta = self.mu.beta
        beta2 = 2*beta - 1 if abs(1 - beta) < 1e-4 else beta**2
        return self.w**2 + 1 - beta2

    @property
    def eps2(self):
        beta = self.mu.beta
        beta2 = 2*beta - 1 if abs(1 - beta) < 1e-4 else beta**2
        return (self.mW2 - self.mN2) * (1 - beta2)
...
class DoubleNu(NuSol):
    ...
    def __init__(self, bs, mus, ev, mW1=mW, mT1=mT, mW2=mW, mT2=mT):
        b, b_ = bs
        mu, mu_ = mus

        sol1 = NuSol(b , mu, ev=None, mW2=mW1**2, mT2=mT1**2 )
        sol2 = NuSol(b_, mu_, ev=None, mW2=mW2**2, mT2=mT2**2)
    ...
    def closest_approach(self):
        H1 = self.solutionSets[0].H_perp
        H2 = self.S.dot(self.solutionSets[1].H_perp)
        min_dist = None 
        min_phi1 = None 
        min_phi2 = None
        for phi1 in np.arange(-3.14, 3.14, 0.1):
            t1 = H1.dot([[np.cos(phi1)], [np.sin(phi1)], [1]])
            for phi2 in np.arange(-3.14, 3.14, 0.1):
                t2 = H2.dot([[np.cos(phi2)], [np.sin(phi2)], [1]])
                dist = ((t2[0][0] - t1[0][0])**2 + (t2[1][0] - t1[1][0])**2 + (t2[2][0] - t1[2][0])**2)**0.5
                if min_dist is None or dist < min_dist:
                    min_dist = dist 
                    min_phi1 = phi1 
                    min_phi2 = phi2
        print(min_dist, min_phi1, min_phi2)
