...
from torch.nn import Sequential as Seq, Linear, Tanh, ReLU, LeakyReLU
...
class RecursiveParticles(MessagePassing):
    ...
    def __init__(self, disable_nu = True, gev = False):
        ...
        self.rnn_edge = Seq(
                ...
                LayerNorm(self._end), ReLU(),
                Linear(self._end, self._repeat),
                Linear(self._repeat, self._repeat)
        )
    ...
    def message(self, edge_index, pmc, trk_i, trk_j, pmc_i, pmc_j, metxy_i):
        ...
        rnn  = self.rnn_edge(feats.to(dtype = torch.float))
        rnn  = self._drop(rnn)
        return rnn
...
class ExperimentalGNNv2(MessagePassing):
    ...
    def __init__(self):
        ...
        self._n = 64
        self.coder = Seq(
                Linear(33, self._n)     , LeakyReLU(0.1),
                Linear(self._n, self._n), LeakyReLU(0.1),
                Linear(self._n, self._n), LayerNorm(self._n),
    ...
    def forward(self,
        ...
        self.O_top_edge = self.rnn_edge(edge_index, batch, pmc, met_xy)
        out = self.O_top_edge.max(-1)[1]
        try: gr = graph.edge(edge_index, out, pmc, True)[1]["node_sum"]
        except KeyError: gr = torch.zeros_like(pmc)

        data = torch.cat([pmc, gr - pmc, gr, pid, pmc[:, :2] - met_xy[batch]], -1)
        aggre = self.propagate(edge_index, tope = out.view(-1, 1), data = data)
        aggre = self._pool(aggre, batch)
        self.O_ntops = self.ntops(aggre)
