...
def MakeParticle(inpt, factor=1.):
    Nu = Neutrino()
    Nu.px = inpt[0] * factor
    Nu.py = inpt[1] * factor
    Nu.pz = inpt[2] * factor
...
for i in range(T.n):
    ...
    numSolutions_Torch.append(len(neutrinos_t))
    
    if not s[i]: continue
    nu_sols = np.array(s[i].nunu_s)
    for k in nu_sols:
        nut1 = MakeParticle(k[0].tolist(), 1000.)
        nut2 = MakeParticle(k[1].tolist(), 1000.)
        neutrinos.append([nut1, nut2])
    numSolutions_Python.append(len(neutrinos))
