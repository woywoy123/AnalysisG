...
def FitToTruth(t1, t2, nus):
    ...
    for i, j in nus:
        ...
        diff += (t1._pz - j.pz)**2
        diff += (t2._pz - i.pz)**2
        fit[diff] = [i, j] # < keep like this, we dont care if the order is incorrect.
...
for i in Ana:
    ...
    if len(tops) == 2:
        ...
        nu1 = [c for c in t1.Children if c.is_nu] 
        nu2 = [c for c in t2.Children if c.is_nu] 
        if len(nu1 + nu2) != 2:
            continue
        
        lep1 = [c for c in t1.Children if c.is_lep]
        lep2 = [c for c in t2.Children if c.is_lep]
        if len(lep1 + lep2) != 2:
            continue
        
        q1 = [ c for c in t1.Children if c.is_lep == False and c.is_nu == False ]
        q2 = [ c for c in t2.Children if c.is_lep == False and c.is_nu == False ]
        if len(q1 + q2) != 2:
            continue
