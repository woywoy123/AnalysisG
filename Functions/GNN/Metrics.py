...
from Functions.Plotting.Histograms import TGraph
...
class Metrics(Directories):
    ...
    def __CreateGraph(self, key, xAxis, yAxis, Title, Index = None):
        L = TGraph()
        L.Color = "Blue" 
        L.xTitle = xAxis
        L.yTitle = yAxis
        L.Title = Title
        L.xMax = max(self.Epochs)+1
        L.xMin = min(self.Epochs)+1
       
        L.Filename = key
        if Index != None:
            L.Filename = key + "_" + str(Index)

        tmp = {}
        for ep in self.Epochs:
            val = self.Stats[ep][key]
            ...
            if key == "FoldTime":
                it = 0
                for k in self.Stats[self.Min]["kFold"][Index]: 
                    if k not in tmp:
                        tmp[k] = val[Index][it]
                    else:
                        tmp[k] += val[Index][it]
        ...
        if key == "FoldTime":
            for k in tmp:
                L.yData.append(float(tmp[k]/len(self.Epochs)))
                L.xData.append(k)
    ...
    def PlotStats(self, PlotDir = None):
        ...
        #self.__CreateGraph("EpochTime", "Epochs", "Time (s)", "Time Elapsed Per Epoch")

        print(self.Stats[1])
        for i in range(len(self.Stats[self.Min]["Nodes"])):
            n = self.Stats[self.Min]["Nodes"][i]
            self.__CreateGraph("FoldTime", "kFold", "Average Time Elapsed", "Nodes-"+str(n) , Index = i)
